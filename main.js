...
function getRequirement(id) {
	if (id < 9 && id > 0) {
		return Math.floor(Math.pow(id+1-getUpgradeEffect(id, 1), (data.prestiges[id][4]+1)/1.5));
	}
	if (id == 0) return Math.floor(Math.pow(1.5-getUpgradeEffect(id, 1), data.prestiges[id][4]/1.5))*10;
...
function upgrade(tier,id) {
	if (data.prestiges[tier+1][0] >= getUpgradeRequirement(tier,id)) {
		data.prestiges[tier+1][0] = 0;
		data.prestiges[tier+1][4] = 0;
...
function getUpgradeEffect(tier, id) {
	if (id == 1) {
		...
		switch(id) {
			case 2:
				return 1+(data.prestiges[tier][2]*(tier+1)/10); 
			break
			case 3:
...
function draw() {
	...
	data.prestiges.forEach(function (el, i) {
		...
		document.getElementById("tier"+(i+1)+"mul").innerHTML = "x"+(Math.floor(1+Math.pow(data.prestiges[i][0], getUpgradeEffect(i, 2))));

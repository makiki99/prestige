...
function getRequirement(id) {
	if (id === 0) {
		...
		return Math.floor(Math.pow(1+((id+1)/3.1*Math.pow(0.8-id*0.05, Math.pow(data.prestiges[id+1],1.5)))),data.prestiges[id]+1)/data.prestiges[id+1]+1)
	} else {
		return Math.floor(Math.pow(1+((id+1)/2.1*Math.pow(0.8-id*0.05, Math.pow(data.prestiges[id+1],1.5)))),data.prestiges[id]+1)/(Math.sqrt(data.prestiges[id+1])+1))
	}
}
